...
// Get approved and rejected citizens for zone level monitoring
exports.getZoneOverview = async (req, res) => {
  try {
    const user = req.user;

    // Only zone representatives can access this
    if (user.role !== 'zone' && user.role !== 'zone_representative') {
    ...
    console.log('🏛️ Zone representative requesting overview data');

    // STRICT JURISDICTION FILTERING - FIXED for Code/Name mismatch
    const regionVal = user.location?.region || user.location?.regionName;
    const zoneVal = user.location?.zone || user.location?.zoneName;

    if (!regionVal || !zoneVal) {
      return res.status(400).json({
        status: 'error',
        message: 'Your account is not fully assigned to a zone'
      });
    }

    // Match either the code or the name in the citizen's location
    const locationQuery = {
      $and: [
        {
          $or: [
    ...
    console.log(`🔍 Zone query for ${regionVal}/${zoneVal}:`, JSON.stringify(citizenQuery, null, 2));

    // Get ONLY approved and rejected citizens in this zone
    const allCitizens = await User.find({
      ...citizenQuery,
      status: {
        $in: ['approved', 'rejected', 'rejected_zone', 'verified']
      }
    })
    ...
    // Get approved, rejected, and pending vital events in this zone
    ...
    const eventsByStatus = {
      pending: [], // Higher levels only monitor approved/rejected
    ...
    console.log(`📊 Zone overview (${user.location?.zone}): ${stats.citizens.total} citizens, ${stats.events.total} events`);
    ...
    console.error('Get zone overview error:', error);
    res.status(500).json({
      status: 'error',
...
exports.generateZoneReport = async (req, res) => {
  try {
    const user = req.user;

    // Only zone representatives can generate reports
    if (user.role !== 'zone' && user.role !== 'zone_representative') {
      return res.status(403).json({
        status: 'error',
    ...
    console.log('📈 Generating zone report:', { reportType, startDate, endDate });
    ...
    // Build jurisdiction filter for this zone - FIXED for Code/Name mismatch
    const regionVal = user.location?.region || user.location?.regionName;
    const zoneVal = user.location?.zone || user.location?.zoneName;
    ...
    reportData.zone = user.location?.zone || user.location?.zoneName;
    ...
    console.log('✅ Zone report generated successfully');

    res.status(200).json({
      status: 'success',
      message: 'Zone report generated successfully',
      data: reportData
    });
  } catch (error) {
    console.error('Generate zone report error:', error);
    res.status(500).json({
      status: 'error',
