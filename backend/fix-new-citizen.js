...
async function fixNewCitizen() {
  try {
    await mongoose.connect('mongodb://localhost:27017/ethiopia-vital-events');
    console.log('✅ Connected to MongoDB');

    // Find citizens with old 'verified' status and update them
    const result = await User.updateMany(
      ...
      { 
        $set: { 
          ...
          kebeleApprovalDate: new Date() // Track when kebele approved
        }
      }
    );
    ...
    pendingWoreda.forEach(c => {
      ...
      console.log('Kebele Approval Date:', c.kebeleApprovalDate);
    });
